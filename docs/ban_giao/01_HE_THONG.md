...
- **Quyền ghi `don_hang` ở tầng API khai theo DANH SÁCH CHO-PHÉP** (WP-11b/QD-96, db/213): grant UPDATE cho `authenticated` = **63 cột viết tay** (không sinh động từ `information_schema` như db/150) → **cột thêm sau mặc định ĐÓNG**. Rớt 5 cột hệ-tự-ghi: `id` · `ma_don` · `tao_luc` · `la_demo` · `sale_phu_trach`. Whitelist: `db/grant_don_hang_whitelist.txt`; test canh: `web/ops/test_grant_don_hang.mjs` (so hai chiều, lệch là ĐỎ nêu tên cột). **Thêm cột nghiệp vụ = viết RPC, KHÔNG nới whitelist cho xanh test.**
...
| 10. Cắt → làm → xong | tho/xuong | Xưởng · Trạm quét | `quet_tem` mỗi bước (đơn theo bước chậm nhất) · **back-flush ván/phụ kiện; trả `back_flush`/`thieu_he_so` → toast xanh (đã xuất) / vàng (thiếu hệ số, tem vẫn chạy) — WP-33/36** | su_kien_quet · don_hang_mon(trang_thai) · phieu(xuat_sx) |
