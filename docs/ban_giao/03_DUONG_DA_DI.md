...
- **QD-92 — Đèn trần CAC mức CHIẾN DỊCH, 5 trạng thái, KHÔNG lộ số trần ra app Quảng cáo (WP-92, db/205).** Đèn con_du/sat_tran/vuot_tran/chua_du_so/khong_do_duoc tính ở DB; biên trần (dải giá trị đơn) ở app Tài chính, RPC ads chỉ trả TRẠNG THÁI. Ba đèn con_du/sát/vượt CHƯA sáng được vì chưa có đường đơn→chiến dịch (chờ CAPI vế a). "XONG CƠ CHẾ" ≠ tag XONG.
- **QD-93 — Ngưỡng cảnh báo ads lưu THAM SỐ có lịch sử (`ads_nguong`), không nhét code (WP-93, db/205→208).** Đổi ngưỡng = đóng khoảng cũ + mở khoảng mới (khuôn QD-68/90), test bẻ ngưỡng chứng minh đọc-từ-bảng. 5 ngưỡng + `vuot_muc_tang_chung_pct`/`gop_canh_bao_khi_tu`/`chi_so_ty_le_dang_ngo` đều `[TẠM]`.
- **BÀI HỌC (ads, L-04→05) — luật cảnh báo "chi tăng" phải so với NHỊP CHUNG, không so với 0.** So chi kỳ này với kỳ trước theo % TĂNG rồi bắt vượt hẳn nhịp cả tài khoản (Garrison ch.10 variance): kỳ trước=0 → `moi_bat` (không chia 0), ≥3 cùng nổ → gộp. So với 0 = mọi chiến dịch mới đều "đột biến" → báo động giả, người tắt cảnh báo.
- **BÀI HỌC (ads, L-05) — bộ kéo Meta từng gán SAI NGÀY do lệch múi giờ −1.** `new Date(pgDate).toISOString()` ở UTC+7 lùi một ngày (local-midnight → UTC hôm trước). Đọc ngày dạng TEXT (`to_char`), dựng UTC-midnight (`str+'T00:00:00Z'`). Số ads trước 02/09 từng dính; `keo_lai_khoang_ads` kéo lại đã vá. Cũng bài học: `luot_bam` Meta = `clicks` (mọi lượt bấm) ≠ `inline_link_clicks` (bấm-vào-link) — CTR/CPC phải theo link.
- **QD-96 — Quyền ghi tầng API khai theo DANH SÁCH CHO-PHÉP; cột nghiệp vụ thêm sau mặc định ĐÓNG; thêm cột thì viết RPC, KHÔNG nới whitelist cho xanh test (WP-11b, db/213).** Grant sinh động là **BỆNH chứ không phải sự cố**: db/150 viết "mọi cột TRỪ trang_thai" nên mỗi cột mới là **một cửa hậu im lặng**. Đã trả giá 2 lần (db/211 `han_tra_loi`, db/212 `ly_do_thua`) trước khi đảo chiều. db/213 grant 63 cột viết tay + test canh (`test_grant_don_hang.mjs`) so hai chiều với `db/grant_don_hang_whitelist.txt`.
- **QD-95 — Cột nghiệp vụ do RPC ghi thì client KHÔNG cầm grant UPDATE (WP-10, db/212).** `ly_do_thua`/`ghi_chu_thua` revoke khỏi `authenticated`; đường ghi duy nhất `doi_trang_thai_don`. **Bệnh, không phải sự cố:** grant db/150 (QD-66) viết theo **danh sách CẤM** ("mọi cột TRỪ trang_thai") nên MỌI cột nghiệp vụ sinh sau (db/036 `ly_do_thua`, db/209 `han_tra_loi`) **tự hở ra tầng API** — WP-72 chặn ở RPC nhưng PATCH thẳng vẫn 200 + không để vết ai (**hệ KHÔNG có sổ ai-sửa-cột-nào** → đóng được từ nay, không truy được quá khứ). `han_tra_loi` đã phải vá riêng (db/211) = lần thứ hai cùng bệnh → nên đảo sang **danh sách CHO-PHÉP** + test canh cột mới. Họ QD-64/66/67.
