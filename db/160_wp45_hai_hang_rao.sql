-- db/160 (WP-45 L-18) — HAI HÀNG RÀO (ERP 7.3.3): demand fence + planning fence · bỏ dời lẻ · gom neo thứ 4.
--   ① Mốc fence THEO ĐƠN: don_hang.khoa_lich_luc/boi — đặt lúc bàn giao (việc 6), CẢ khi xếp không được (QD-69).
--   ② Demand fence: luu_xep_lich(p_tu_dong=true) gặp đơn đã khoá → {ok:false, ly_do:'DA_KHOA_LICH'}, KHÔNG raise.
--   ③ Planning fence: người bấm — trước cho_cat tự do; từ cho_cat BẮT BUỘC vai ceo/xuong + lý do.
--   ④ DROP tl_doi_viec (dời lẻ) — không hàm DB nào gọi (UI sẽ gỡ nút ở lô sau).
--   ⑤ nut_that_ghi: neo xuôi thứ 4 → gọi neo_xuoi().
begin;

-- ① cột mốc fence theo đơn (KHÔNG tái dùng moc_lich)
alter table kho.don_hang add column if not exists khoa_lich_luc timestamptz;
alter table kho.don_hang add column if not exists khoa_lich_boi uuid;

-- ④ bỏ dời lẻ (không hàm DB nào gọi — đã rà)
drop function if exists kho.tl_doi_viec(bigint, date, boolean, text);

-- ②③ luu_xep_lich: đổi chữ ký (+p_tu_dong) → drop bản 4 tham số rồi tạo bản 5 tham số
drop function if exists kho.luu_xep_lich(text, text, boolean, text);
CREATE OR REPLACE FUNCTION kho.luu_xep_lich(p_ma_don text, p_kieu text, p_ngoai_le boolean DEFAULT false, p_ly_do text DEFAULT NULL::text, p_tu_dong boolean DEFAULT false)
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'kho'
AS $function$
declare v_vai text; v_khoa timestamptz; v_tt text; r jsonb; e jsonb; i int; v_tuan date; v_vung text; n int := 0;
begin
  v_vai := coalesce(kho.current_vai_tro(),'');
  if p_tu_dong then
    -- [WP-45 ② DEMAND FENCE] máy gọi: đơn đã KHOÁ LỊCH → BỎ QUA, KHÔNG raise, KHÔNG ghi (chạy hàng loạt không được vỡ)
    select khoa_lich_luc into v_khoa from kho.don_hang where ma_don = p_ma_don;
    if v_khoa is not null then
      return jsonb_build_object('ok', false, 'ly_do', 'DA_KHOA_LICH', 'ma_don', p_ma_don);
    end if;
  else
    -- người bấm: gác vai + [WP-45 ③ PLANNING FENCE]
    ...
    -- từ cho_cat trở đi (đã vào sản xuất) → BẮT BUỘC có lý do; trước cho_cat → xếp lại tự do
    if v_tt in ('cho_cat','da_cat','dang_lam','xong_sx','cho_giao','da_giao')
       and coalesce(btrim(p_ly_do),'') = '' then
      raise exception 'luu_xep_lich: đơn đã vào sản xuất (%) — xếp lại BẮT BUỘC có lý do', v_tt;
    end if;
  end if;
  ...
  if p_kieu='nguoc' then
    ...
    r := kho._sched(p_ma_don,'xuoi', kho.neo_xuoi());
  end if;
...
grant execute on function kho.luu_xep_lich(text, text, boolean, text, boolean) to authenticated;

-- ① ban_giao_xuong: việc 6 đặt khoa_lich (CẢ khi da_xep=false)
...
-- ⑤ nut_that_ghi: neo thứ tư → neo_xuoi()
CREATE OR REPLACE FUNCTION kho.nut_that_ghi()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'kho'
...
begin
  ...
  for d in select distinct ma_don from kho.xep_lich loop
    ...
    else r := kho._sched(d.ma_don,'xuoi', kho.neo_xuoi()); end if;
...
-- HOÀN TÁC: chạy lại db/159 (luu_xep_lich 4-arg, ban_giao_xuong, nut_that_ghi) + db/079 (tl_doi_viec);
--   alter table kho.don_hang drop column khoa_lich_luc, drop column khoa_lich_boi;
