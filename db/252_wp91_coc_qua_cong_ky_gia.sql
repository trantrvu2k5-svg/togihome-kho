-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- 252 — WP-91 P1 (CEO quyết 09/09): ngưỡng CỌC (cửa chặn bàn giao) chọn kỳ QUA CỔNG kho.ky_gia_hien_hanh()
--   như 13 hàm giá — KHÔNG lấy theo THÁNG DƯƠNG LỊCH (to_char(current_date,'YYYY-MM')) nữa.
--   Lý do (lời CEO): cọc chặn bàn giao = chặn TIỀN + chặn SẢN XUẤT; không để kỳ chép [TẠM] chưa ai soát điều khiển.
--   PHẠM VI ĐÚNG 2 HÀM (db/174): ban_giao_xuong · vuot_coc_canh_bao.
--   • GỠ nền 30 CHÔN TRONG CODE (coalesce …, 30) ở CẢ HAI. Thiếu kỳ → RAISE, KHÔNG im lặng.
--   • GIỮ column default 30 của tham_so_tai_chinh.coc_toi_thieu_du_an_pct — đó là GIÁ TRỊ khởi tạo kỳ mới
--     (kỳ luôn có coc), KHÔNG phải nền im lặng ở tầng gọi.
--   • ban_giao_xuong (đường nóng): chỉ đổi ĐÚNG biểu thức chọn kỳ + thêm RAISE; KHÔNG đụng thứ tự 7 việc,
--     KHÔNG đụng nhánh cửa-vượt-CEO-ghi-vết. HOÀN TÁC: chạy lại bản db/174.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
begin;

-- ══════════ vuot_coc_canh_bao — SQL → plpgsql (để RAISE khi thiếu kỳ) ══════════
create or replace function kho.vuot_coc_canh_bao(p_ma_don text)
 returns boolean language plpgsql stable security definer set search_path to 'kho' as $function$
declare v_pct numeric; d kho.don_hang;
begin
  select * into d from kho.don_hang where ma_don = p_ma_don;
  if d.ma_don is null or d.vuot_coc_boi is null then return false; end if;   -- chưa vượt cọc → không cảnh báo (khỏi cần ngưỡng)
  v_pct := (select coc_toi_thieu_du_an_pct from kho.tham_so_tai_chinh where ma_ky = kho.ky_gia_hien_hanh());  -- [WP-91 P1] kỳ ĐÃ XÁC NHẬN
  if v_pct is null then raise exception 'THIEU_NGUONG_COC: không có coc_toi_thieu_du_an_pct cho kỳ giá "%"', kho.ky_gia_hien_hanh(); end if;
  return coalesce((select sum(so_tien) from kho.phieu_thu where ma_don = p_ma_don and loai = 'coc'), 0)
    < round(kho.gia_don(d.gia_chot, d.doanh_thu, d.gia_cong_thuc) * v_pct / 100.0, 0);
end $function$;

-- ══════════ ban_giao_xuong — CHỈ đổi khối v_pct (cửa cọc), phần còn lại GIỮ NGUYÊN ══════════
...
begin
  ...
  --   [WP-91 P1] Ngưỡng đọc theo KỲ GIÁ ĐÃ XÁC NHẬN qua kho.ky_gia_hien_hanh() (như 13 hàm giá) — KHÔNG theo tháng dương lịch.
  --   Thiếu kỳ → RAISE (đã GỠ nền 30 chôn trong code; column default 30 của tham_so GIỮ nguyên = giá trị kỳ mới).
  if coalesce(v_don.dong,'') = 'du_an' then
    v_pct := (select coc_toi_thieu_du_an_pct from kho.tham_so_tai_chinh where ma_ky = kho.ky_gia_hien_hanh());
    if v_pct is null then
      raise exception 'THIEU_NGUONG_COC: không có coc_toi_thieu_du_an_pct cho kỳ giá "%" — không xác định ngưỡng cọc (kiểm tham_so_tai_chinh)', kho.ky_gia_hien_hanh();
    end if;
