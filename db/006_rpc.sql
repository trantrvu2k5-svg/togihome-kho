-- KHO-1 — RPC ghi (security definer): cập nhật ton/lo_nhap + thẻ kho ATOMIC. Web app gọi qua supabase.rpc.
--   App KHÔNG được update ton trực tiếp (chỉ grant select). Ghi đi qua 2 hàm này, KIỂM vai trò BÊN TRONG.
--   3 tháng đầu: tồn âm VẪN ghi + cờ 'ton_am' (không chặn).
begin;

-- ── Thợ quét tem: lấy ra / trả về ──
create or replace function kho.quet_giao_dich(p_vat_tu_ma text, p_loai text, p_so_luong numeric)
  returns numeric language plpgsql security definer set search_path = kho as $$
declare vt text; vid uuid; kid uuid; moi numeric; delta numeric;
begin
  vt := current_vai_tro();
  if vt is null then raise exception 'Chưa đăng nhập'; end if;
  if vt not in ('ceo','kho','tho') then raise exception 'Không có quyền'; end if;
  if p_loai not in ('lay','tra') then raise exception 'loai phải lay/tra'; end if;
  if p_so_luong is null or p_so_luong <= 0 then raise exception 'Số lượng phải > 0'; end if;
  select id into vid from vat_tu where ma = p_vat_tu_ma;
  if vid is null then raise exception 'Không có mã %', p_vat_tu_ma; end if;
  select id into kid from kho where la_mac_dinh limit 1;
  delta := case when p_loai='tra' then p_so_luong else -p_so_luong end;
  insert into ton(vat_tu_id,kho_id,so_luong) values(vid,kid,delta)
    on conflict (vat_tu_id,kho_id) do update set so_luong = ton.so_luong + delta, sua_luc = now()
    returning so_luong into moi;
  insert into giao_dich(vat_tu_id,kho_id,loai,so_luong,so_du_sau,nguon,canh_bao,nguoi_thao_tac)
    values(vid,kid,p_loai,delta,moi,'quet_tem', case when moi<0 then 'ton_am' end,
           (select id from nguoi_dung where auth_uid = auth.uid()));
  return moi;
end $$;
grant execute on function kho.quet_giao_dich(text,text,numeric) to authenticated;

-- ── CEO/kho ghi sổ phiếu nhiều dòng: sinh số phiếu, cập nhật tồn (BQGQ), lô, thẻ kho ──
...
begin
  vt := current_vai_tro();
  if vt not in ('ceo','kho') then raise exception 'Chỉ CEO/kho được ghi sổ'; end if;
