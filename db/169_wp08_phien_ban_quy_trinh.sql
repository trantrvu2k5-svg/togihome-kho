-- db/169 (WP-08 L-02) — PHIÊN BẢN MẪU QUY TRÌNH + NEO THEO MÓN.
--   Vá lỗ khảo sát: món trỏ thẳng MẪU CHUNG, không snapshot; không bảng lịch sử sửa mẫu (lỗ ③).
--   TẦNG NỀN thôi — CHƯA đổi hàm đọc sang helper (để L-03), TRỪ ban_giao_xuong (mục 6, neo phiên bản).
begin;

-- ═══ 1. quy_trinh_buoc: cột phien_ban + đổi UNIQUE ═══
alter table kho.quy_trinh_buoc add column if not exists phien_ban int not null default 1;
comment on column kho.quy_trinh_buoc.phien_ban is
  'WP-08: phiên bản mẫu. buoc_truoc[] TRỎ thu_tu → chép phiên bản phải GIỮ NGUYÊN thu_tu, KHÔNG đánh số lại.';
alter table kho.quy_trinh_buoc drop constraint if exists quy_trinh_buoc_ma_quy_trinh_thu_tu_key;
alter table kho.quy_trinh_buoc drop constraint if exists quy_trinh_buoc_ma_pb_thu_tu_key;
alter table kho.quy_trinh_buoc add constraint quy_trinh_buoc_ma_pb_thu_tu_key unique (ma_quy_trinh, phien_ban, thu_tu);

-- ═══ 2. bảng phiên bản (ĐỒNG THỜI là lịch sử phát hành — lỗ ③) ═══
create table if not exists kho.quy_trinh_phien_ban (
  ma_quy_trinh    text        not null references kho.quy_trinh(ma_quy_trinh) on delete cascade,
  phien_ban       int         not null,
  trang_thai      text        not null check (trang_thai in ('hien_hanh','cu')),
  phat_hanh_luc   timestamptz not null default now(),
  nguoi_phat_hanh text,
  ly_do           text,
  primary key (ma_quy_trinh, phien_ban)
);
create unique index if not exists uq_qtpb_hien_hanh on kho.quy_trinh_phien_ban(ma_quy_trinh) where trang_thai = 'hien_hanh';
comment on table kho.quy_trinh_phien_ban is 'WP-08: phiên bản mẫu + lịch sử phát hành (lỗ ③). Mỗi ma_quy_trinh chỉ MỘT dòng hien_hanh.';
insert into kho.quy_trinh_phien_ban(ma_quy_trinh, phien_ban, trang_thai, ly_do)
  select distinct ma_quy_trinh, 1, 'hien_hanh', 'backfill WP-08' from kho.quy_trinh_buoc
  on conflict (ma_quy_trinh, phien_ban) do nothing;

-- ═══ 3. don_hang_mon: cột neo + FK (client KHÔNG ghi — không grant cột này, họ QD-67) ═══
alter table kho.don_hang_mon add column if not exists quy_trinh_phien_ban int;
comment on column kho.don_hang_mon.quy_trinh_phien_ban is
  'WP-08: phiên bản quy trình NEO lúc bàn giao. NULL=chưa bàn giao→đọc bản hien_hanh. Client KHÔNG ghi (họ QD-67).';
alter table kho.don_hang_mon drop constraint if exists dhm_quy_trinh_pb_fkey;
alter table kho.don_hang_mon add constraint dhm_quy_trinh_pb_fkey
  foreign key (ma_quy_trinh, quy_trinh_phien_ban) references kho.quy_trinh_phien_ban(ma_quy_trinh, phien_ban);
-- KHÔNG cho client ghi cột mới. Grant don_hang_mon là TABLE-WIDE (revoke cột vô hiệu) → chặn bằng
-- TRIGGER (họ QD-64/66: server sở hữu cột). Server neo qua ban_giao_xuong (SECURITY DEFINER, current_user=owner).
create or replace function kho._chan_ghi_qt_phien_ban()
 returns trigger language plpgsql as $fn$
begin
  if current_user in ('authenticated','anon')
     and (case when tg_op = 'INSERT' then new.quy_trinh_phien_ban is not null
               else new.quy_trinh_phien_ban is distinct from old.quy_trinh_phien_ban end) then
    raise exception 'quy_trinh_phien_ban do SERVER neo lúc bàn giao — client không được ghi (WP-08)';
  end if;
  return new;
end $fn$;
drop trigger if exists trg_chan_qt_phien_ban on kho.don_hang_mon;
create trigger trg_chan_qt_phien_ban before insert or update on kho.don_hang_mon
  for each row execute function kho._chan_ghi_qt_phien_ban();

-- ═══ 5. Hai helper (MỘT nguồn suy duy nhất — luật suy rút từ ban_giao_xuong) ═══
create or replace function kho.quy_trinh_cua_mon(p_mon_id uuid)
 ...
 language sql stable security definer set search_path to 'kho' as $fn$
  ...
  where m.id = p_mon_id
$fn$;
grant execute on function kho.quy_trinh_cua_mon(uuid) to authenticated;

create or replace function kho.buoc_cua_mon(p_mon_id uuid)
 returns setof kho.quy_trinh_buoc
 language sql stable security definer set search_path to 'kho' as $fn$
  select b.* from kho.quy_trinh_buoc b, kho.quy_trinh_cua_mon(p_mon_id) qm
  where b.ma_quy_trinh = qm.ma_quy_trinh and b.phien_ban = qm.phien_ban
  order by b.thu_tu
$fn$;
grant execute on function kho.buoc_cua_mon(uuid) to authenticated;

-- ═══ 4. Backfill neo: món ĐÃ BÀN GIAO (đã CHỐT GIỜ) → phien_ban=1. ma_quy_trinh NULL → để NULL (báo sau) ═══
update kho.don_hang_mon m set quy_trinh_phien_ban = 1
  where m.quy_trinh_phien_ban is null and m.ma_quy_trinh is not null
    and exists (select 1 from kho.quy_trinh_phien_ban pb where pb.ma_quy_trinh = m.ma_quy_trinh and pb.phien_ban = 1)
    and exists (select 1 from kho.so_don_vi_mon s where s.mon_id = m.id and (s.gio_moi_don_vi_chot is not null or s.gio_co_dinh_chot is not null));

-- ═══ 6. ban_giao_xuong: neo phiên bản hien_hanh lúc chốt (chèn VIỆC 3b, không đổi gì khác) ═══
