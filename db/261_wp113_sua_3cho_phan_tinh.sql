-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- 261 — WP-113 L-113-3b: 3 sửa trước khi vẽ màn.
--   [1] ads_suc_khoe_mau.ket_luan_ma KHỚP 1–1 với nhánh ket_luan (7 mã, thêm xep_hang_duoi_tb + chua_ro_thuong_hieu).
--   [2] p_brand (default null) cho ads_bang_ky + ads_viec_phai_lam → lọc thương hiệu SERVER (như ads_tong_so_sanh).
--       mau_can_doi đếm ĐÚNG mẫu khối Sức khoẻ tô "cần xem lại" = ket_luan_ma <> 'dang_tot' (ads.js:498).
--   [3] ads_bang_ky mỗi dòng web: lien_he_chi_tu_quang_cao = cờ brand; cờ true → gia_1_lien_he NULL (không so được),
--       luot_lien_he giữ số (= cuộc trò chuyện).
--   CHỈ THÊM/sửa key MỚI của db/260 — key cũ giữ nguyên. Param p_brand có DEFAULT → caller 2-tham-số cũ KHÔNG gãy.
--   HOÀN TÁC: chạy lại db/260.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
begin;

-- p_brand thêm vào ads_bang_ky/ads_viec_phai_lam → BẢN 2 THAM SỐ cũ (db/260) phải DROP, nếu không caller 2-arg
--   khớp bản cũ (thiếu field mới) do exact-match thắng bản có DEFAULT. Bản 3-arg-default phục vụ cả gọi 2-arg.
drop function if exists kho.ads_bang_ky(date, date);
drop function if exists kho.ads_viec_phai_lam(date, date);

-- ══════════════════ [1] ads_suc_khoe_mau — ket_luan_ma 1–1 (7 mã) ══════════════════
...
begin
  ...
  k as (
    select t.*,
      ...
      -- [WP-113 3b] MÃ 1–1 với NHÁNH trên (7 mã). KHÔNG gộp hai câu khác nhau.
      case
        when t.brand = 'chưa rõ' then 'chua_ro_thuong_hieu'
...
-- ══════════════════ [2]+[3] ads_bang_ky — p_brand (lọc server) + lien_he_chi_tu_quang_cao ══════════════════
...
begin
  ...
  j as (select j0.*, coalesce((v_lien_brand ->> brand)::boolean, null) co_flag
        from j0 where p_brand is null or j0.brand = p_brand)   -- [WP-113 2] lọc thương hiệu SERVER
  select
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      -- [WP-113 3] cờ true (thương hiệu 0 sự kiện liên hệ 30 ngày) → gia_1_lien_he NULL (không so được)
      'gia_1_lien_he',     case when coalesce(co_flag,false) then null
      ...
      'web_chua_co_su_kien_lien_he', co_flag,
      'lien_he_chi_tu_quang_cao', co_flag
...
-- ══════════════════ [2] ads_viec_phai_lam — p_brand (lọc dòng CŨ + MỚI) + mau_can_doi = ket_luan_ma <> dang_tot ══════════════════
...
begin
  ...
  with cur as (select campaign_id, max(campaign_name) ten, max(objective) obj, sum(chi_tieu) chi, sum(cuoc_tro_chuyen) cuoc, min(ngay) tu,
       ...
       cls as (
         ...
         where p_brand is null or c.brand = p_brand),   -- [WP-113 2] lọc thương hiệu (dòng cũ + mới cùng bộ lọc)
  ...
  -- [3a] keo_do (account-wide, KHÔNG lọc brand — cảnh báo số cho mọi thương hiệu)
  ...
  -- [3b/3c] gia_cuoc_cao + loi_web (chặn chung: tuổi<ad_moi_du_ngay HOẶC hiển thị<hien_thi_min → không kết luận) + lọc brand
  with camp as (
  ...
  enr as (select camp.*, seen.tuoi, bc.brand, (nm.chi / nullif(nm.cuoc,0)) nen_gia_cuoc
    ...
    where p_brand is null or bc.brand = p_brand),   -- [WP-113 2] lọc brand cho dòng mới
  ...
  -- [3d] mau_can_doi: đếm mẫu khối Sức khoẻ tô "cần xem lại" = ket_luan_ma <> 'dang_tot' (lọc brand) + số chiến dịch.
  ...
  with m as (select (e->>'ad_id') ad_id, (e->>'ten') ten from jsonb_array_elements(v_suc->'mau') e
             where (e->>'ket_luan_ma') <> 'dang_tot' and (p_brand is null or (e->>'brand') = p_brand)),
  ...
  select case when (select count(*) from m) > 0 then jsonb_build_array(jsonb_build_object(
      ...
      'cau',(select count(*) from m)||' mẫu cần xem lại (khối Sức khoẻ) ở '||(select count(*) from nm)||' chiến dịch — mở bảng xem từng cái.'))
